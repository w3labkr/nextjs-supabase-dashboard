...
create or replace function set_favorite(pid bigint, uid uuid, isfavorite boolean)
returns void
security definer set search_path = public
as $$
begin
  if exists (select 1 from favorites where post_id = pid and user_id = uid) then
    update favorites set is_favorite = isfavorite where post_id = pid and user_id = uid;
  else
    insert into favorites(post_id, user_id, is_favorite) values(pid, uid, isfavorite);
  end if;
end;
$$ language plpgsql;
